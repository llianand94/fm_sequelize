...
module.exports.getAllGroups = async (req, res, next) =>{
  try{
    ...
    if( !groups  ){
      return next(createHttpError(404, 'Groups were not found'));
      }
...
module.exports.getUserGroups = async (req,res,next) => {
  try{
    ...
    if(!userWithGroups){
      return next(createHttpError(404, 'Groups with current user not found'));
      }
...
module.exports.getUsersInGroup = async (req,res,next) => {
  try{
    const {groupInstance,params:{groupId}} = req;
    const groupWithUsers = await groupInstance.getUsers({
        attributes:{
    ...
    if( !groupWithUsers ){
      return next(createHttpError(404, 'Current group with  users not found'));
      }
...
module.exports.deleteUserFromGroup = async (req,res,next)=>{
  try{
    ...
    if(!result){
      return next(createHttpError(404, 'Group with this user was not found!'));
    };
...
module.exports.updateGroup = async (req,res,next)=>{
  try{
    ...
    if(!updatedGroup){
      return next(createHttpError(404, 'Could not update group'));
    };
